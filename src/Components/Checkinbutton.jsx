...
const Checkinbutton = ({todayRecord,onAction}) => {
    ...
    const handleAttendance = async () => {
       setLoading(true);
       try {
        await api.post("/attendance/clock-time");
        onAction();
       } catch (error) {
        toast.error(error?.response?.data?.error || "Attendance Failed")
       } finally{
        setLoading(false)
       }
    }
    ...
    <div className='absolute bottom-4 right-4 flex flex-col z-1'>
     <button onClick={handleAttendance} disabled={loading} className={`w-full max-w-xs cursor-pointer flex justify-between items-center gap-8 p-4 rounded-xl bg-linear-to-br text-white ${isCheckedIn ? "from-slate-700 to-slate-900":"from-indigo-600 to-indigo-700"}`}>
